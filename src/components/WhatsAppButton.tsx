...
type WhatsAppButtonProps = {
  // true SOLO en la pagina de Propietarios: hace que el clic dispare la
  // conversion de Google Ads "click a WhatsApp" de la campaña de
  // propietarios (ver TrackedWhatsAppLink). Este mismo botón flotante se
  // usa también en Huéspedes y Quiénes somos -- ahí NO debe contar como esa
  // conversión (son consultas de huéspedes, no leads de propietarios), por
  // eso el default es false.
  trackConversion?: boolean;
};

const BUTTON_CLASSNAME =
  "group fixed bottom-6 left-6 z-50 flex items-center gap-3 rounded-full bg-[#25D366] py-3 pl-3 pr-3 text-white shadow-lg shadow-black/15 transition-all hover:pr-5";

const ICON_AND_LABEL = (
  <>
...
export default function WhatsAppButton({ trackConversion = false }: WhatsAppButtonProps) {
  if (trackConversion) {
    return (
      <TrackedWhatsAppLink
        href={WHATSAPP_URL}
